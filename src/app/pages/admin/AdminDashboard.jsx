...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState([
    { title: 'Total Resources', value: '0', icon: FolderOpen, gradient: 'from-blue-500 to-blue-600', bgColor: 'bg-blue-100', link: '' },
    { title: 'Total Users', value: '0', icon: Users, gradient: 'from-red-500 to-red-600', bgColor: 'bg-red-100', link: '' },
    { title: 'Feedback Count', value: '0', icon: MessageSquare, gradient: 'from-sky-500 to-sky-600', bgColor: 'bg-sky-100', link: '' },
  ]);
  ...
  useEffect(() => {
    async function loadDashboard() {
      try {
        ...
        setStats([
          { title: 'Total Resources', value: String(resourceStats.totalResources || 0), icon: FolderOpen, gradient: 'from-blue-500 to-blue-600', bgColor: 'bg-blue-100', link: `${baseUrl}/Resource-Management` },
          ...
          { title: 'Feedback Count', value: String(feedbackStats.total || 0), icon: MessageSquare, gradient: 'from-sky-500 to-sky-600', bgColor: 'bg-sky-100', link: `${baseUrl}/Feedback-Review` },
        ]);
      } catch (error) {
    ...
    <AdminLayout>
      <div className="space-y-8 min-h-screen">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="relative">
          <motion.div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-700 rounded-full text-sm font-medium mb-4" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}>
              ...
              <motion.div key={stat.title} onClick={() => navigate(stat.link)} className="group relative bg-white rounded-2xl shadow-lg p-8 cursor-pointer overflow-hidden border border-gray-100" initial={{ opacity: 0, y: 30, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6, delay: 0.1 * index }} whileHover={{ y: -10 }}>
                ...
                <div className="relative">
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <div className="size-10 bg-gray-100 rounded-xl flex items-center justify-center group-hover:bg-blue-100 transition-colors">
                      <TrendingUp className="size-5 text-gray-600 group-hover:text-blue-600 transition-colors" />
                    </div>
                  </div>
